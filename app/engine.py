...
async def reconcile_with_timeout(do_reconcile_coro):
    """
    Wrap reconcile in lock + timeout. If a previous reconcile is hung longer
    than RECONCILE_TIMEOUT_S, log it explicitly and force a new attempt.
    """
    ...
    if _reconcile_lock.locked():
        ...
        if running_s > RECONCILE_TIMEOUT_S:
            log.warning("reconcile_timeout_exceeded running_s=%.1f limit=%d forcing_new",
    ...
    async with _reconcile_lock:
        ...
        try:
            await asyncio.wait_for(do_reconcile_coro(), timeout=RECONCILE_TIMEOUT_S)
