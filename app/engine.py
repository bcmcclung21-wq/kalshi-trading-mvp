    ...
    async def _run_cycle_inner(self) -> None:
        ...
        for m in markets_with_books:
            ...
            if not snap:
                ...
                raw_payload = (book.get("raw") or book) if isinstance((book.get("raw") or book), dict) else {}
                market_data_keys = sorted(list((raw_payload.get("marketData") or {}).keys())) if isinstance(raw_payload.get("marketData"), dict) else []
                logger.info(
                    "liquidity_skip ticker=%s reason=no_snapshot_or_quotes yes_bids=%d yes_asks=%d no_bids=%d no_asks=%d yes_bid=%.4f yes_ask=%.4f no_bid=%.4f no_ask=%.4f raw_keys=%s market_data_keys=%s",
