...
def _normalize_levels(raw: Any) -> list[dict[str, float]]:
    """Convert Kalshi orderbook levels [['0.15', '100.00'], ...] to [{price, quantity}, ...].
    Handles both the v2 _dollars/_fp string format and legacy dict format."""
    ...
    for lvl in raw:
        try:
            if isinstance(lvl, dict):
                price = float(lvl.get("price") or lvl.get("price_dollars") or 0.0)
                qty = float(lvl.get("quantity") or lvl.get("count") or lvl.get("size") or 0.0)
            ...
            continue
        if 0.0 < price < 1.0 and qty > 0:
            out.append({"price": price, "quantity": qty})
    return out


def _normalize_orderbook(raw: dict[str, Any]) -> dict[str, Any]:
    """Convert any Kalshi orderbook response to canonical {ticker, yes: [...], no: [...]} shape.
    Kalshi v2 returns {orderbook_fp: {yes_dollars: [...], no_dollars: [...]}}.
    Legacy/test fixtures may return flat {yes: [...], no: [...]}.
    Both shapes are handled."""
    if not raw:
        return {"yes": [], "no": []}
    fp = raw.get("orderbook_fp") or raw.get("orderbook") or {}
    yes_raw = (fp.get("yes_dollars") if fp else None) or raw.get("yes_bids") or raw.get("yes") or []
    no_raw = (fp.get("no_dollars") if fp else None) or raw.get("no_bids") or raw.get("no") or []
    out = {
        "ticker": raw.get("ticker") or raw.get("market_ticker"),
        "yes": _normalize_levels(yes_raw),
        "no": _normalize_levels(no_raw),
    }
    return out
...
class KalshiClient:
    ...
    async def get_orderbook(self, ticker: str) -> dict[str, Any]:
        ...
        # Kalshi v2 wraps the book in {"orderbook": {"orderbook_fp": {...}}} or {"orderbook_fp": {...}}.
        # Normalize to canonical {ticker, yes, no} shape.
        raw = payload.get("orderbook") or payload
        if isinstance(raw, dict) and "ticker" not in raw:
            raw = {**raw, "ticker": ticker}
        normalized = _normalize_orderbook(raw)
        self.cache.set(key, normalized, ttl_seconds=TUNING.orderbook_cache_ttl_sec)
        return normalized
        ...
        while i < len(clean):
            ...
            try:
                ...
                for row in books:
                    ...
                    if ticker:
                        normalized = _normalize_orderbook({**row, "ticker": ticker})
                ...
                if exc.response is not None and exc.response.status_code == 400:
                    for ticker in chunk:
                        ...
                        try:
                            ...
                            if books:
                                row = books[0]
                                out[ticker] = _normalize_orderbook({**row, "ticker": ticker})
                                recovery_count += 1
                            else:
                                failed_tickers += 1
