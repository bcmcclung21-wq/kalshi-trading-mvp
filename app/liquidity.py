...
def compute_snapshot(
    ...
    # Midpoint: prefer two-sided yes; fall back to whichever exists.
    ...
    # Spread: real distance if both sides present; otherwise cap at 1.0 cleanly.
    ...
    if len(state.spread_history) > 1:
        ...
        spread_range = spread
    stability = 1.0 / (1.0 + spread_range)
    ...
    # Use additive floors so a market with real depth but a wide/one-sided spread
    # still produces a non-zero score and reaches the selector's R0-R10 gates,
    # which are the authoritative bad_spread / bad_price / extreme_price rejecters.
    raw_score = spread_score * depth_score * max(0.1, replenishment_score) * max(0.1, stability_score)
    floor_score = 0.01 * depth_score  # small but non-zero whenever there is real depth
